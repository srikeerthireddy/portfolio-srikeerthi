...
const Navbar = () => {
  ...
  return (
    <motion.nav
      className={`fixed top-0 left-0 w-full z-50 px-6 py-4 transition-all duration-300 ${scrolled ? 'bg-black backdrop-blur-sm shadow-lg' : 'bg-black'}`}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 0.3 }}
    >
      <div className="container mx-auto flex justify-between items-center">
        <a href="#" className="text-2xl font-bold text-white">Sri<span className="text-pink-500">Keerthi</span></a>
          ...
          {navItems.map((item) => (
            <li key={item.id}>
              <button 
                ...
                className={`transition duration-300 ${activeSection === item.id ? 'text-pink-500 font-semibold' : 'text-white hover:text-pink-500'}`}
              >
                {item.name}
              </button>
            </li>
